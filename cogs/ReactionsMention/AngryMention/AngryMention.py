...
class AngryMention(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot

        with open('database/emojis.json', 'r', encoding='utf-8') as f: data = json.load(f)
        self.emojis = data['angrymention']

        self.react = app_commands.ContextMenu(name="angrymention", callback=self.react_callback)
...
async def setup(bot):
    await bot.add_cog(AngryMention(bot))