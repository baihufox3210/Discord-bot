...
class RandomMention(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot

        with open('database/emojis.json', 'r', encoding='utf-8') as f: data = json.load(f)
        self.emojis = data['all_emojis']

        self.react = app_commands.ContextMenu(name="隨機添加 20 個表情", callback=self.react_callback)
...
async def setup(bot):
    await bot.add_cog(RandomMention(bot))